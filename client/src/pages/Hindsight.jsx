...
const Hindsight = () => {
  ...
  const [tickers, setTickers] = useState(
    isHindsightTickersFresh(user?.id) ? getHindsightTickersCache() : [],
  );
  ...
  const applyHistoryToState = useCallback((hist, interval, price, tickerList) => {
    ...
    const latest = tickerList.find((t) => t.ticker === selectedTicker);
  ...
  const loadHistory = async () => {
    ...
    if (silent) {
      applyHistoryToState(cached.history, cached.historyInterval, cached.price, tickers);
      return;
    }
    setLoading(true);
    try {
      ...
      applyHistoryToState(hist, interval, r.data.price, tickers);
    } catch {
